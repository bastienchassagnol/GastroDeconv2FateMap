...
Using a combination of bulk transcriptome, single-cell, imaging, and deconvolution algorithms, predict the phenotypic endpoint of gastruloids from early stages
...
[DVC](https://dvc.org/) and stored on a shared remote. Git holds
small `.dvc` pointer files; the actual data files live on the remote
at `/mnt/DATA_11TB/projects/dtoo_project/dvc-remotes/GastroDeconv2FateMap`.
...
**Step 2 — Authorise the public key** on the storage host. Ask a
project maintainer to add your `~/.ssh/id_ed25519_mmg.pub`, or run
(if you have access):
...
**Step 4 — Configure DVC locally** (creates or updates `.dvc/config.local`):
...
If the remote directory is already visible on your filesystem (same
NFS mount or local path), skip SSH entirely:
